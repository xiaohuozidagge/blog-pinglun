(function () {
  ...
  async function restoreBatchContext() {
    ...
    console.log('[AutoComment] restoreBatchContext batchCtx:', ctx);
  ...
  async function handleBatchTaskForAutoMode() {
    ...
    try {
      ...
      console.log('[AutoComment] handleBatchTaskForAutoMode cachedCopy:', !!cachedCopy, 'lastGeneratedPromotionCopy:', !!lastGeneratedPromotionCopy);
      if (!promotionText) {
        console.log('[AutoComment] handleBatchTaskForAutoMode 无缓存文案，返回');
        return;
      }
  ...
  async function autoGeneratePromotionOnPageLoad() {
    ...
    if (_batchCtx) {
      console.log('[AutoComment] autoGeneratePromotionOnPageLoad 进入批处理模式');
    ...
    if (inCooldown) {
      ...
      if (cachedCopy) {
        ...
        console.log('[AutoComment] 冷却期内，填充缓存文案');
        tryFillCommentTextareaWithPromotion(cachedCopy);
        focusCommentTextareaWithPromotion(cachedCopy);
      } else {
        // 无缓存但有评论框，尝试生成
        const hasBox = !!findLikelyCommentTextarea({ allowGenericFallback: false });
        if (hasBox) {
          autoGeneratedOnce = true;
          ...
          if (text) {
            ...
            const shouldAutoSubmit = await getAutoSubmitSetting();
            if (shouldAutoSubmit) {
              setTimeout(() => autoSubmitComment(), 500);
            } else {
              setStatus('生成完成！文案已填入评论框', '#22c55e');
            }
          }
        }
      }
